...
public class DogService extends Service {
    ...
    protected final int onStart(Intent intent, int flags, int startId) {
        ...
        if (!DaemonEnv.initialized) return START_STICKY;
        ...
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.N) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2)
                DaemonEnv.startServiceSafely(new Intent(DaemonEnv.app, WatchDogNotificationService.class));
        }

        //定时检查 AbsWorkService 是否在运行，如果不在运行就把它拉起来
        //Android 5.0+ 使用 JobScheduler，效果比 AlarmManager 好
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            JobInfo.Builder builder = new JobInfo.Builder(HASH_CODE, new ComponentName(DaemonEnv.app, SnapService.class));
            ...
            Intent i = new Intent(DaemonEnv.app, DaemonEnv.sServiceClass);
            sPendingIntent = PendingIntent.getService(DaemonEnv.app, HASH_CODE, i, PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    protected void onEnd(Intent rootIntent) {
        ...
        DaemonEnv.startServiceMayBind(DaemonEnv.sServiceClass);
        DaemonEnv.startServiceMayBind(DogService.class);
    }
    ...
    public static void cancelJobAlarmSub() {
        if (!DaemonEnv.initialized) return;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            JobScheduler scheduler = (JobScheduler) DaemonEnv.app.getSystemService(JOB_SCHEDULER_SERVICE);
            scheduler.cancel(HASH_CODE);
        } else {
            AlarmManager am = (AlarmManager) DaemonEnv.app.getSystemService(ALARM_SERVICE);
    ...
    public static class WatchDogNotificationService extends Service {
        ...
        public int onStartCommand(Intent intent, int flags, int startId) {
            startForeground(DogService.HASH_CODE, new Notification());
            stopSelf();
            return START_STICKY;
        }
