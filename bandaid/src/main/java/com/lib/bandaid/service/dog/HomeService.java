...
public abstract class HomeService extends Service {
    ...
    public static void cancelJobAlarmSub() {
        ...
        DaemonEnv.app.sendBroadcast(new Intent(WakeReceiver.ACTION_CANCEL_JOB_ALARM_SUB));
    }
    ...
    protected int onStart(Intent intent, int flags, int startId) {
        ...
        if (mFirstStarted) {
            ...
            getPackageManager().setComponentEnabledSetting(new ComponentName(getPackageName(), DogService.class.getName()), PackageManager.COMPONENT_ENABLED_STATE_ENABLED, PackageManager.DONT_KILL_APP);
        }
    ...
    public static class WorkNotificationService extends Service {
        ...
        @Override
        public int onStartCommand(Intent intent, int flags, int startId) {
            startForeground(HomeService.HASH_CODE, new Notification());
            stopSelf();
            return START_STICKY;
        }
